...
async def handle_duo_command(inPuTMsG: str, sender_uid: int, chat_id: int, chat_type: int,
    ...
    api_url = f"https://fuhx-rizer-duo.onrender.com/info?uid={target_uid}"
...
# FUCK YOU BRADAR ( RIZER x64 defin i fuck u bruh )
...
def load_credentials_from_file(filename="RAFIN.txt"):
    """
    Load UID and password from RAFIN.txt file
    """
    try:
        if not os.path.exists(filename):
...
async def handle_rizak_on_leave(uid, chat_id, key, iv, chat_type):
    ...
    try:
        ...
        secret_code = f"RIZER_{int(datetime.now().timestamp())}_{random.randint(1000, 9999)}"
        ...
        if rizak_packet and online_writer:
            ...
            print(f"✅ tahmid packet sent on team leave/kick for UID: {uid}")

            # Send notification message
            notify_msg = "[B][C][00FF00]🔄 tahmid packet activated!\n[FFFFFF]Bot left team or was kicked."
...
📱 Player will see:
"RAFIN FF  wants to join your team"
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]TAHMID FF [00FF00]!!
"""
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="rizer", title_id=None):
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="rizer"):
    ...
    try:
        ...
        nested_details.Clan_Name = "TAHMID EMPIRE"  # CHANGE: Your clan
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="RAFIN", title_id=title_id)
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg =  f"""[B][C][FFFF00] এই শা🤫লা বো👽কা👽চু👽দা তুই RAFIN কে নুব বলছিস তোর মাকে কন👽ডম চু👽দি 
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if True:
                            ...
                            try:
                                ...
                                bot_uid = 2579372095
                                message_text = f" Can't accept Your request Talk to RAFIN"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FFD700]★══════════════★
[B][C][DA70D6]    ✦  RAFIN BOT - স্বাগতম  ✦
[B][C][FFD700]══════════════
[B][C][00FFFF]▣ BOT OWNER    : [C][FFFFFF]RAFIN
[B][C][00FFFF]▣ টেলিগ্রাম   : [C][00BFFF]@rafintxt
[B][C][00FFFF]▣ ইউটিউব     : [C][00BFFF]@rafintxtBot_360X
[B][C][FFD700]★══════════════★"""
...
[b][ffdd00][c]◉━━━━━━━━━━━━━━◉
[FFDDAA]     স্বা🥰গত🥰ম TA🥰H🥰M I🥰D এ💀র BOT এ
[b][ffdd00] 👽 BOT OWNER : [FFFFFF]RAFIN
[DDDDDD]👽 TIKTOK USER👽NA👽ME : [55DDAA]@tahmis_hi
...
"""
                                ...
                                credentials = load_credentials_from_file("RAFIN.txt")
                                if not credentials:
...
[B][C][FF0000]TELEGRAM CHANNEL: [FFFFFF]t.me/tahmid_apis

[B][C][FF0000]TELEGRAM GROUP: [FFFFFF]@pp9PMdKS0mw2NGJl
"""
...
[B][00FFFF]Example: /ghost 4337272 TAHMID
"""
...
"""
                                ...
                                try:
                                    url = f"https://rizerxfuhx-tc-offline.onrender.com/join?teamcode={teamcode}"
                                    r = requests.get(url, timeout=10)
...
"""
                                ...
                                try:
                                    url = f"http://rizerxfuhx-tc-offline.onrender.com/joinfuhx?teamcode={teamcode}"
                                    r = requests.get(url, timeout=10)
                                    data = r.text
...
#GALI SPAM MESSAGE 
    ...
    # Add your actual name
                        ...
                        # =================== COMPLETE HELP MENU (SEPARATE PARTS) ===================
                         ...
                         # =================== WORKING COMMANDS HELP MENU ===================
                        if inPuTMsG.strip().lower() in ("help", "/help", "halp", "jisan", "***"):
                            print(f"Help command detected from UID: {uid} in chat type: {XX}")
                            try:
                                basic = """[B][FFD700] ───≪ RAFIN TxT BOT ≫───
 
[B][FFD700]  » Invite Player
[B][FFFFFF]    ↳ /inv
[B][FFD700]  » 3 Player Group
[B][FFFFFF]    ↳ /3
[B][FFD700]  » 4 Player Group
[B][FFFFFF]    ↳ /4
[B][FFD700]  » 5 Player Group
[B][FFFFFF]    ↳ /5
[B][FFD700]  » 6 Player Group
[B][FFFFFF]    ↳ /6
[B][FFD700]  » 5 Player Squad
[B][FFFFFF]    ↳ /snd [uid]
...
[B][FFD700] ─────────────"""
                                ...
                                # --- 2. EMOTES & JOIN (Sky Blue & White) ---
                                emotes = """[B][00FFFF] ───≪ RAFIN TxT BOT ≫───

[B][00FFFF]  » Spam Invite
[B][FFFFFF]    ↳ /spam_group [uid]
[B][00FFFF]  » Join by UID
[B][FFFFFF]    ↳ /join_req [uid]
[B][00FFFF]  » Join by Code
[B][FFFFFF]    ↳ /join [tc]
[B][00FFFF]  » Exit Squad
[B][FFFFFF]    ↳ /exit
[B][00FFFF]  » Fast Emote
...
[B][00FFFF] ─────────────"""
                                ...
                                # --- 3. EVO EMOTES (Purple & White) ---
                                evo = """[B][DA70D6] ───≪ RAFIN TxT BOT ≫───

[B][DA70D6]  » Auto Evo Emote
[B][FFFFFF]    ↳ @evos
[B][DA70D6]  » Off Evo Emote
[B][FFFFFF]    ↳ @sevos
[B][DA70D6]  » Evo to Player
[B][FFFFFF]    ↳ /evo [uid] [num]
[B][DA70D6]  » Evo Single
[B][FFFFFF]    ↳ /Evo [name]
[B][DA70D6]  » Number to Emote
[B][FFFFFF]    ↳ [0-4]18
[B][DA70D6]  » No-Bot Emote
[B][FFFFFF]    ↳ /em [tc] [num]
...
[B][DA70D6] ─────────────"""
                                ...
                                # --- 4. FUN COMMANDS (Golden & White) ---
                                fun_cmnd = """[B][FFD700] ───≪ RAFIN TxT BOT ≫───

[B][FFD700]  » Start Match
[B][FFFFFF]    ↳ /start1
[B][FFD700]  » Duo Check by UID
[B][FFFFFF]    ↳ /duo [uid]
[B][FFD700]  » Respect by Name
[B][FFFFFF]    ↳ /সম্মান [name]
[B][FFD700]  » Lonewolf 1v1
[B][FFFFFF]    ↳ /lw [tc]
[B][FFD700]  » Love by Name
[B][FFFFFF]    ↳ /love [name]
[B][FFD700]  » Gender Check
[B][FFFFFF]    ↳ /gender [name]

[B][FFD700] ─────────────"""
                                await safe_send_message(response.Data.chat_type, fun_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                # --- 5. SPAM COMMANDS (Sky Blue & White) ---
                                spam_cmnd = """[B][00FFFF] ───≪ RAFIN TxT BOT ≫───

[B][00FFFF]  » Magic Colour
[B][FFFFFF]    ↳ /magic
[B][00FFFF]  » Call Me Noob
[B][FFFFFF]    ↳ /noob
[B][00FFFF]  » Send Title
[B][FFFFFF]    ↳ /title
[B][00FFFF]  » Send Sticker
[B][FFFFFF]    ↳ /sticker
[B][00FFFF]  » Freeze UID
[B][FFFFFF]    ↳ /freeze [uid]
...
[B][00FFFF] ─────────────"""
                                ...
                                # --- 6. BADGES COMMANDS (Purple & White) ---
                                badge_cmnd = """[B][DA70D6] ───≪ RAFIN TxT BOT ≫───
...
[B][DA70D6] ─────────────"""
                                ...
                                # --- 7. ADVANCED COMMANDS (Golden & White) ---
                                ad_cmnd = """[B][FFD700] ───≪ RAFIN TxT BOT ≫───

[B][FFD700]  » Normal Gali
[B][FFFFFF]    ↳ /gop [name]
[B][FFD700]  » Danger Gali
[B][FFFFFF]    ↳ /gopgop [name]
[B][FFD700]  » Respect Cmd
[B][FFFFFF]    ↳ /সম্মান [name]
[B][FFD700]  » Style Text
[B][FFFFFF]    ↳ /mg [text]
[B][FFD700]  » Bundle Auto
[B][FFFFFF]    ↳ /magicbundle [tc]
[B][FFD700]  » Ghost Join
[B][FFFFFF]    ↳ /ghost [tc] [name]

[B][FFD700] ─────────────"""
                                await safe_send_message(response.Data.chat_type, ad_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                # --- 8. INFO COMMANDS (Sky Blue & White) ---
                                info_cmnd = """[B][00FFFF] ───≪ RAFIN TxT BOT ≫───

[B][00FFFF]  » Offline Team
[B][FFFFFF]    ↳ /offline [tc]
[B][00FFFF]  » Evo Bundle List
[B][FFFFFF]    ↳ /b
[B][00FFFF]  » Evo Animation
[B][FFFFFF]    ↳ /animation
[B][00FFFF]  » Evo Combo Bundle
[B][FFFFFF]    ↳ /look
...
[B][00FFFF] ─────────────"""
                                ...
                                # --- 9. SECURITY & ROOMS (Purple & White) ---
                                about = """[B][DA70D6] ───≪ RAFIN TxT BOT ≫───

[B][DA70D6]  » Emote Block
[B][FFFFFF]    ↳ /block_emote [uid]
[B][DA70D6]  » Emote Unblock
[B][FFFFFF]    ↳ /unblock_emote [uid]
[B][DA70D6]  » Player Team Info
[B][FFFFFF]    ↳ /status [uid]
[B][DA70D6]  » Invite Lag ID
[B][FFFFFF]    ↳ /TAHMIDlag [uid]
[B][DA70D6]  » Join Room
[B][FFFFFF]    ↳ /xjoin [room_id] [pass]

[B][DA70D6] ─────────────"""
                                await safe_send_message(response.Data.chat_type, about, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                # --- 10. WHITELIST & SYSTEM (Golden & White) ---
                                whitelist_cmnd = """[B][FFD700] ───≪ RAFIN TxT BOT ≫───

[B][FFD700]  » Coin Toss Game
[B][FFFFFF]    ↳ /coin
[B][FFD700]  » Bangla Dhadha
[B][FFFFFF]    ↳ /dhadha
[B][FFD700]  » UID Info
[B][FFFFFF]    ↳ /info [uid]
[B][FFD700]  » Admin Info
[B][FFFFFF]    ↳ /admin
[B][FFD700]  » Kick Player
[B][FFFFFF]    ↳ /kick [uid]
[B][FFD700]  » Note: Bot must be leader

[B][FFD700] ─────────────"""
                                await safe_send_message(response.Data.chat_type, whitelist_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                # --- 11. PREMIUM INFO (Sky Blue & White) ---
                                last = """[B][00FFFF] ───≪ RAFIN TxT BOT ≫───

[B][FFFFFF]  » YOUTUBE ID
[B][00FFFF]    ↳ @rafintxtBot_360X
[B][FFFFFF]  » TELEGRAM ID
[B][00FFFF]    ↳ @rafintxtBot_360X
                        ...
                        if original_msg and "StickerStr" in original_msg:
                            ...
                            if NUMBER_EMOTES:
                                ...
                                try:
                                    ...
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H)
                                    # Also send a message
                                    sticker_reply = "[B][C][FFFF00] RAFIN STYLE TCP"
...
async def handle_addfriend_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 2:
        ...
        return
    target_uid = parts[1]
    # Load credentials from RAFIN.txt
    credentials = load_credentials_from_file("RAFIN.txt")
    if not credentials or len(credentials) < 2:
        error_msg = f"[B][C][FF0000]❌ Failed to load credentials from RAFIN.txt\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
...
async def handle_world_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        async def GenJoinGlobaL(owner, code, is_rizer_gay=True, K=key, V=iv):
            fields = {
                1: 4,
                2: {
                    1: owner,
...
async def handle_worlds_command(uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        ...
        async def GenJoinGlobaL(owner, code, is_rizer_gay=True, K=key, V=iv):
            ...
            return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), '0515', K, V)
        packet1 = await GLobaL("RizerisGay", key, iv)
...
# =================== NEW FUNCTION: /TAHMIDlag LOOP (fixed: no leave at end, added small delays) ===================
chatspam_running = False
chatspam_task = None
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from RAFIN.txt...")
    credentials = load_credentials_from_file("RAFIN.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create RAFIN.txt with your UID and password")
